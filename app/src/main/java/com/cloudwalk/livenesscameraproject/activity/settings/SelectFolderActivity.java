...
import com.cloudwalk.livenesscameraproject.fileselect.FolderAdapter;
import com.cloudwalk.livenesscameraproject.fileselect.FolderInfo;
import com.cloudwalk.livenesscameraproject.fileselect.MediaFileUtil;
...
public class SelectFolderActivity extends Activity implements
    ...
    private SelectFolderActivity mContext;
    private String mKey;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_select_folder_para_set);

        initView();
        refreshListItems("/sdcard");
    }

    private void initView() {
        ivBack = (ImageView) findViewById(R.id.iv_file_setting_back);
        ...
        //Intent传递的路径
        mKey = getIntent().getStringExtra("SP_KEY");
    }
    ...
    private OnClickListener comfirmListener = new OnClickListener() {
        ...
        public void onClick(View arg0) {
            Toast.makeText(mContext, (String) fullPath.getText(), Toast.LENGTH_SHORT).show();
    ...
    private List<FolderInfo> getFolderList(String path) {
        ...
        if (!rootDirectory.equals(path)) {
            ...
            fi.setFolderIcon(R.drawable.ic_folder);
            fi.setFolderName("...");
            fi.setFolderPath("");
        ...
        for (File file : files) {
            ...
            if (file.isDirectory() && file.getName().indexOf(".") != 0) {
                ...
                fol.setFolderIcon(R.drawable.ic_folder);
                ...
                if (MediaFileUtil.isImageFileType(file.getPath())) {
                    FolderInfo fol = new FolderInfo();
                    fol.setFolderIcon(R.drawable.ic_file);
                    fol.setFolderName(file.getName());
                    fol.setFolderPath(file.getPath());
                    fol.setFolderTime(longDateToString(file.lastModified()));
    ...
    public void onItemClick(AdapterView<?> parent, View v, int position, long id) {
        if (folderlist.get(position).getFolderPath().isEmpty())
            ...
            if (file.isDirectory()) {
                ...
                String path = fullPath.getText() + "/" + folderlist.get(position).getFolderName();
                Intent intent = new Intent();
                intent.putExtra("path", path);
                intent.putExtra("key", mKey);
                // 设置返回码和返回携带的数据
